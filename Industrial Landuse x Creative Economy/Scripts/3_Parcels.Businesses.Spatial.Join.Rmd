...
nbhds <- st_read("K:/DataServices/Datasets/Boundaries/Spatial/bos_qcy_svl_cam_arl_nbhds/partnercity_nbhds.shp")
...
hp_biz <- mapc_biz.spatial |> 
  filter(
    CONAME == "North River Graphics Inc" & STCITY_CLEAN == "Pembroke"
  )
...
ggplot() +
  geom_sf(data = waltham_parcels, aes(fill = ADDR_NUM))+
  geom_sf(data = hp_biz) +
  theme_minimal()
```
2.4 Norwood
...
hp_biz <- mapc_biz.spatial |> 
  filter(
    CONAME == "Diamond Guild" & STCITY_CLEAN == "Norwood"
  )
...
map.1 <- ggplot() +
  ...
  geom_sf(data = points) +
  geom_sf(data = muni_boundary, fill = NA, color = "black", linewidth = 0.5) +
  geom_sf(data = fab_dist, fill = NA, aes(color = ZoneCode), linewidth = 0.8) +
  scale_color_discrete(
...
#Set Muni we want to evaluate (as a proof of concept)
set_muni <- "Waltham"
...
#Second we calculate a Moran's I statistic for each parcel based on the density of 
