...
hp_parcel <- mapc_parcels |> 
  filter(
    LOC_ID == "F_727286_2959936"
  )
...
rsp_biz <- mapc_biz.spatial |> 
  filter(
    CONAME == "Red Spot Printing" & STCITY_CLEAN == "Waltham"
  )

waltham_parcels <- mapc_parcels |> 
  filter(
    CITY == "Waltham" & (FULL_STR == "NEWTON ST" | FULL_STR == "CALVARY ST")
  )
...
#Here we have an example of the parcel data not having am address (or parcel) that matches the address of the business (182 Newton St vs 4 Calvary St).
```

3.0 Tabular Analysis
```{r}
#Use codes
Biz_LUC <- biz_in_parcel |> 
  group_by(
    Min_LUC
  ) |> 
  summarise(
