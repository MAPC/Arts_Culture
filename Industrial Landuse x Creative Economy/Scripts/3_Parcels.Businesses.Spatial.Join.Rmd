...
```{r}
#Set the muninipality that you would like to map
set_muni <- ""
...
points <- mapc_biz.spatial |> 
  filter(
    STCITY_CLEAN == "Watertown" & OBJECT_ID != "169672"
  )

map.1 <- ggplot() +
  geom_sf(data = waltham_parcels_full, fill = NA, color = "grey")+
  geom_sf(data = points) +
  geom_sf(data = muni_boundary, fill = NA, color = "black", linewidth = 0.5) +
  labs(
    title = "Point Location of Arts Businesses in Watertown (2023)"
  ) +
  theme_minimal() +
  theme(
...
ggsave(
  filename = "BizPoints_Watertown.jpeg",
  plot = map.1,
  device = "jpeg",
  path = output_path
)
...
map.2 <- ggplot() +
  geom_sf(data = waltham_parcels_full, linewidth = 0.001, aes(fill = biz), color = "grey") +
  ...
  geom_sf(data = waltham_parcels_ind, aes(color = USE_CODE_SYMB), fill = NA, linewidth = 0.8) +
  scale_color_discrete(
    name = "Industrial Parcels"
  ) +
  geom_sf(data = muni_boundary, fill = NA, color = "black", linewidth = 0.5) +
  labs(
    title = "Density of Arts Businesses in Watertown (2023)"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_blank(),
...
ggsave(
  filename = "BizDensity_Watertown.jpeg",
  plot = map.2,
  device = "jpeg",
  path = output_path
)
...
map.3 <- ggplot() +
  geom_sf(data = waltham_parcels_full, aes(fill = as.factor(ind_intersect)), color = "grey") +
  scale_fill_manual(
    values = c("white", "purple"),
    name = "Industrial Parcels\nwith Arts Businesses"
  ) +
  geom_sf(data = muni_boundary, fill = NA, color = "black", linewidth = 0.5) +
  labs(
    title = "Intersect of Arts Businesses & Industrial Parcels in Watertown (2023)"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_blank(),
...
ggsave(
  filename = "BizIndIntersect_Watertown.jpeg",
  plot = map.3,
  device = "jpeg",
  path = output_path
)
