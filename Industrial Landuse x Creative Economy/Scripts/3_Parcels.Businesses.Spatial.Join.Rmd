...
biz_in_muni <- st_join(mapc_biz.spatial, mapc_municipalities, join = st_within)
```
...
# Table of 
Industrial.Parcel_Biz <- biz_in_parcel |> 
  mutate(USE_CODE_SYMB = case_when(
    USE_CODE_SYMB == "Industrial Vacant" ~ "Industrial",
    USE_CODE_SYMB == "Commercial Vacant" ~ "Commercial",
    USE_CODE_SYMB == "Single Family Residence" ~ "Residential",
    USE_CODE_SYMB == "Two or Three Family Residences" ~ "Residential",
    USE_CODE_SYMB == "Residential Other" ~ "Residential",
    USE_CODE_SYMB == "Residential Vacant" ~ "Residential",
    USE_CODE_SYMB == "Apartments with Four or more units" ~ "Residential",
    .default = USE_CODE_SYMB
  )) |>
  group_by(
    muni_name,
    USE_CODE_SYMB
...
tabular_parcels <- mapc_parcels |> 
  ...
  group_by(muni_name, USE_CODE_SYMB) |> 
  summarise(
    num_parcels = n()
  ) |> 
  ungroup() |> 
  ...
  mutate(
    tot_parcels = sum(num_parcels)
  ) |> 
  ungroup() |> 
  mutate(
    pct_parcels = num_parcels/tot_parcels
  )

x <- left_join(
  tabular_parcels,
  Industrial.Parcel_Biz,
  by = c("muni_name", "USE_CODE_SYMB")
) |> 
  mutate(
    across(everything(), ~replace_na(.x, 0)),
    pct_Parcels_w_Arts.Biz = arts.biz/num_parcels
  )

# Industrial
x.industrial <- x |> 
  filter(
    USE_CODE_SYMB %in% c("Industrial")
  ) |> 
  mutate(
    rank = (pct_Parcels_w_Arts.Biz - min(pct_Parcels_w_Arts.Biz))/ (max(pct_Parcels_w_Arts.Biz) - min(pct_Parcels_w_Arts.Biz))
  ) |> 
  select(
    muni_name,
    pct_Parcels_w_Arts.Biz,
    rank
  )

x.industrial.spatial <- inner_join(
  mapc_municipalities,
  x.industrial,
  by = c("muni_name")
)
...
map.test <- ggplot() +
  geom_sf(data = x.industrial.spatial, aes(fill = pct_Parcels_w_Arts.Biz), color = "black")+
  scale_fill_continuous(high = "purple", low = "white", name = "% Industrial Parcels/nwith Arts Businesses") +
  labs(
    title = paste0("Percentage of Industrial Parcels with Arts Businesses (2023)")
  )+
  theme_minimal()+
  theme(
    axis.text.x = element_blank(),
...
x.commercial.spatial <- inner_join(
  mapc_municipalities,
  x.commercial,
  by = c("muni_name")
)
...
map.test <- ggplot() +
  geom_sf(data = x.commercial.spatial, aes(fill = pct_Parcels_w_Arts.Biz), color = "black")+
  scale_fill_continuous(high = "purple", low = "white", name = "% Commercial Parcels/nwith Arts Businesses") +
  labs(
    title = paste0("Percentage of Commercial Parcels with Arts Businesses (2023)")
  )+
  theme_minimal()+
  theme(
    axis.text.x = element_blank(),
...
x.res <- x |> 
  filter(
    USE_CODE_SYMB %in% c("Residential")
  ) |> 
  mutate(
    rank = (pct_Parcels_w_Arts.Biz - min(pct_Parcels_w_Arts.Biz))/ (max(pct_Parcels_w_Arts.Biz) - min(pct_Parcels_w_Arts.Biz))
  ) |> 
...
map.test <- ggplot() +
  geom_sf(data = x.res.spatial, aes(fill = pct_Parcels_w_Arts.Biz), color = "black")+
  scale_fill_continuous(high = "purple", low = "white", name = "% Residential Parcels/nwith Arts Businesses") +
  labs(
    title = paste0("Percentage of Residential Parcels with Arts Businesses (2023)")
  )+
  theme_minimal()+
  theme(
    axis.text.x = element_blank(),
...
munis <- mapcdatakeys::all_muni_data_keys |> filter(mapc == 1 & !muni_name %in% c("Hamilton", "Manchester-by-the-Sea")) |>  select(muni_name) |> pull()

I_rook.list <- NULL
Moran.test_rook.list <- NULL

I_queen.list <- NULL
Moran.test_queen.list <- NULL
  
for (i in munis) {
  
  print(paste0("Testing Spatial Autocorrelation for ", i,"."))
  #Create a separate set of parcel data for the test municipality
  ...
  # First, determine the neighbor matrix for each parcel using the rook and queen method.
  rook_nhbrs <- spdep::poly2nb(cluster_anlaysis_parcels, queen = FALSE)
  ...
  #being considered is a function of that value on the other spatial object. #Values closer to 0
  #indicate two spatial objects are not alike.
  # 1. Rook Moran's I stat generation
  I_rook <- moran(cluster_anlaysis_parcels$biz, lw_rook, length(rook_nhbrs), Szero(lw_rook))
  print(I_rook[1])
  
  # Collects the Moran I stat for the municipality being considered into a vector which will be aggregated later for review.
  I_rook.list <- bind_rows(I_rook.list, I_rook[1])
  
  # 2. Queen Moran's I stat generation
  I_queen <- moran(cluster_anlaysis_parcels$biz, lw_queen, length(queen_nhbrs), Szero(lw_queen))
  print(I_queen[1])
  
    # Collects the Moran I stat for the municipality being considered into a vector which will be aggregated later for review.
   I_queen.list <- bind_rows(I_queen.list, I_queen[1])
  
  # Fourth we conduct a hypothesis test. It works like a one sided t-test.
  # 1. Rook method hypothesis test
  mt.r <- moran.test(cluster_anlaysis_parcels$biz, lw_rook, alternative = "greater")
  
  # Print the summary of the test.
  print(mt.r)
  
    # Collects the p value for the Moran I stat hypothesis test for the municipality being considered into a vector which will be aggregated later for review. 
  Moran.test_rook.list <- bind_rows(Moran.test_rook.list, mt.r[2])
  
  # 2. Queen method hypothesis test
  mt.q <- moran.test(cluster_anlaysis_parcels$biz, lw_queen, alternative = "greater")
  
  # Print the summary of the test.
  print(mt.q)
  
    # Collects the p value for the Moran I stat hypothesis test for the municipality being considered into a vector which will be aggregated later for review. 
  Moran.test_queen.list <- bind_rows(Moran.test_queen.list, mt.q[2])
  
}

# Create a dataframe of muni names to bind to the associated stat vectors from the Moran's I tests in the for loop above.
muni_names <- data.frame(munis)

# Bind columns of muni names, Moran's I statistics, and p values from the Moran Tests for each municipality created in the for loop above.
test.stat.df <- bind_cols(
  muni_names,
  I_rook.list, Moran.test_rook.list,
  I_queen.list, Moran.test_queen.list
) |> 
  # Rename columns for comprehension.
  dplyr::rename(
    Rook_MoranI = I...2,
    Rook_MoranTest_pval = p.value...3,
    Queen_MoranI = I...4,
    Queen_MoranTest_pval = p.value...5,
  )

# General gist -
# MAPC as a region - yes businesses tend to cluster in parcels but those clusters are dispersed throughout the region, there isnt one area that's a central locus for arts businesses.
# Individual municipalities - It depends! Some do not exhibit clustering phenomenon, some do! Some places the clusters are concentrated - I stat closer to 1, others more dispersed - i stat closer to -1. And some places where the businesses tend to be evenly spread throughout the parcels in the municipality.
